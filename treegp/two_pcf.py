    ...
    if A.__class__ in [
        ...
        if A.__class__ in [sklearn.gaussian_process.kernels.Product]:
            ...
            for key in A.__dict__:
                if A.__dict__[key].__class__ in [
                    treegp.kernels.AnisotropicVonKarman,
                    treegp.kernels.AnisotropicRBF,
                ]:
            ...
            if not ok:
                raise ValueError(
                    "Work only with treegp.kernels.AnisotropicVonKarman and treegp.kernels.AnisotropicRBF"
                )
        else:
...
class robust_2dfit(object):
    ...
    def minimize_minuit(self, p0=[3000.0, 0.2, 0.2]):
        """
        ...
        if not self._fit_ok:
            ...
            size = np.linspace(p0[0] - p0[0] / 10.0, 2 * p0[0], N_restart)
...
class two_pcf(object):
    ...
    def __init__(
        ...
        if self.ndim not in [1, 2]:
            raise ValueError(
                "two-pcf support only 1d and 2d modeling for the moment. curent ndim: %i"
                % (self.ndim)
            )
        if self.ndim == 2:
            self.X = X
    ...
    def comp_2pcf(self, X, y, y_err):
        ...
        if self.anisotropic:
            ...
            dy = (kk.bottom_edges + kk.top_edges) / 2.0
            dx = dy.T
            ...
            cat = treecorr.Catalog(x=X[:, 0], y=X[:, 1], k=(y - np.mean(y)), w=w)
            kk = treecorr.KKCorrelation(
                min_sep=self.min_sep, max_sep=self.max_sep, nbins=self.nbins
            )
            kk.process(cat)
            distance = kk.meanr
            mask = np.ones_like(kk.xi, dtype=bool)
    ...
    def comp_xi_covariance(self, n_bootstrap=1000, mask=None, seed=610639139):
        ...
        xi_cov = 1.0 / (len(dxi) - 1.0) * np.dot(dxi.T, dxi)
        return xi_cov
    ...
    def return_2pcf(self, seed=610639139):
        ...
        if self.anisotropic:
            ...
            xi_cov = self.comp_xi_covariance(
                n_bootstrap=int(results[0]), mask=mask, seed=seed
            )
            bias_factor = (int(results[0]) - 1.0) / (
                int(results[0]) - len(xi[mask]) - 2.0
            )
            xi_weight = np.linalg.inv(xi_cov) * bias_factor
        else:
            # let like developed initialy for the moment
            xi_weight = np.eye(len(xi)) * 1.0 / np.var(self.y)
    ...
    def optimizer(self, kernel):
        ...
        if self.robust_fit:
            robust = robust_2dfit(
                kernel, xi, coord[:, 0], coord[:, 1], xi_weight, mask=mask
            )
