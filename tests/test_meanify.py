...
def test_meanify():
    ...
    meanify = treegp.meanify(bin_spacing=40., statistics='mean')
    for n in range(nfields):
        meanify.add_field(coords[n], fields[n])
    meanify.meanify()
    meanify.save_results(directory='output',
                         name_output='mean_gp_stat_mean.fits')
...
def test_gpinterp_meanify():
    ...
    for n, opt in enumerate(optimizer):
        ...
        gp = treegp.GPInterpolation(kernel=kernel, optimize=True,
                                    ...
                                    max_sep=3.,
                                    average_fits='output/mean_gp_stat_mean.fits')
